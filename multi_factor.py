...
使用 yfinance 替代 akshare（Yahoo Finance 支持 .SS/.SZ A股代码，无访问限制）
"""
import logging, time
...
# ── 候选 A 股池（沪深300 + 中证500 精选，Yahoo Finance .SS/.SZ 格式）
CANDIDATE_STOCKS = [
...
# ── 技术指标（纯 Python，不依赖 pandas/ta-lib）
def _ema(data, n):
    k, r = 2/(n+1), [data[0]]
...
def run_selector(weights, top_n=10, stock_pool=None):
    ...
    if stock_pool:
        pool=[(c+(".SS" if c.startswith("6") else ".SZ"), c, "") for c in stock_pool if "." not in c]
        pool+=[(c,c,"") for c in stock_pool if "." in c]
    ...
    for sym,name,*_ in pool:
        try:
            ...
            results.append(StockScore(
                code=sym.split(".")[0], name=name, price=price,
                change_pct=chg, total_score=tot, factor_scores=fs,
                buy_reason=_reason(fs), risk_tip=_risk(fs), pe=pe, volume_ratio=vr
            ))
            time.sleep(0.15)
...
def _demo(n):
    ...
    return [StockScore(code=c,name=nm,price=p,change_pct=ch,
            total_score=sc,buy_reason=r,risk_tip="风险适中") for c,nm,p,ch,sc,r in d[:n]]
